...
func _process(delta: float) -> void:
    ...
    if player_in_range_god and Input.is_action_just_pressed("interaction"):
        ...
        press_f = true
        interaction = true
    if interaction and press_f and player_in_range_god and Input.is_action_just_pressed("1"):
        press_f = false
        run_dialogue("ending 2")
    if interaction and god_chat == false and player_in_range_god and Input.is_action_just_pressed("2"):
        cloud_shop_toggle()
