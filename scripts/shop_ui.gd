...
func update_shop_display() -> void:
    ...
    if Global.crops.has("Corn") and corn_text:
        ...
        corn_stock_text.text = "%d" % Global.crops["Corn"]["stock"]
    ...
    if Global.crops.has("Milk") and milk_text:
        var milk_price = Global.crops["Milk"]["current_price"]
        milk_text.text = "%d" % milk_price
        milk_stock_text.text = "%d" % Global.crops["Milk"]["stock"]
        if current_money < milk_price:
            milk_text.modulate = Color.RED
        else:
            milk_text.modulate = Color.WHITE
            
    if Global.crops.has("Chocolate") and chocolate_text:
        var chocolate_price = Global.crops["Chocolate"]["current_price"]
        chocolate_text.text = "%d" % chocolate_price
        chocolate_stock_text.text = "%d" % Global.crops["Chocolate"]["stock"]
...
func _on_corn_button_pressed():
    ...
    if Global.subtract_money(price):
        ...
        MarketStock.refresh_price("Corn")
        update_shop_display()
        Global.inventory_ui.refresh()
        Global.hotbar_ui.refresh()
    else:
...
func _on_milk_button_pressed() -> void:
    var price = Global.crops["Milk"]["current_price"]
    if Global.subtract_money(price):
        if buy_sound:
            buy_sound.play()
        update_ui()
        add_to_inventory("Milk")
        Global.crops["Milk"]["stock"] -= 1
        MarketStock.refresh_price("Milk")
        update_shop_display()
        Global.inventory_ui.refresh()
        Global.hotbar_ui.refresh()
    else:
...
func _on_chocolate_button_2_pressed() -> void:
    var price = Global.crops["Chocolate"]["current_price"]
    if Global.subtract_money(price):
        if buy_sound:
            buy_sound.play()
        update_ui()
        add_to_inventory("Chocolate")
        Global.crops["Chocolate"]["stock"] -= 1
        MarketStock.refresh_price("Chocolate")
        update_shop_display()
        Global.inventory_ui.refresh()
        Global.hotbar_ui.refresh()
    else:
